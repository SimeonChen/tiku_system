...
function openCreateCatalogDialog($node) {
    ...
    $then.find("#parentId").val(doc_id);
    $then.find("#docId").val('');
    $then.find("#documentName").val('');
...
function openDeleteDocumentDialog($node) {
    var index = layer.confirm('你确定要删除该文档吗？', {
        ...
        $.post(window.deleteURL,{"identify" : window.book.identify,"id" : $node.id}).done(function (res) {
            layer.close(index);
            if(res.code === 0){
...
function openEditCatalogDialog($node) {
    ...
    $then.find("#documentName").val(text);
    $then.find("#docId").val(doc_id);
    $then.find("#parentId").val(parentId);
    
  //  $then.find("input[name='docId']").val(doc_id);
